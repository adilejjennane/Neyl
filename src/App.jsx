...
// ------------------------------------------------------------
// TrigoTrainer — App de révision trigonométrie (Terminale FR)
// - Modes : Flashcards • Cercle (visuel) • Étapes guidées • Équations • Progression
// - Mobile-first, sauvegarde locale, ludique
// ------------------------------------------------------------

// --- Utilitaires radians / affichage ---
const PI = Math.PI;
const TAU = 2 * Math.PI;
...
function radToNiceLabel(rad) {
  // Essaie d'exprimer l'angle comme k * PI / q, pour q in {1..12}
  const k = rad / PI;
  const tol = 1e-6;
  for (let q = 1; q <= 12; q++) {
  ...
  // sinon valeur numérique en rad arrondie
  return `${(rad).toFixed(2)} rad`;
}

function labelToRad(label) {
  // Accepte formes: aπ/b, -aπ/b, aπ, π/2, etc. et aussi nombres déc.
  const s = label.replaceAll(" ", "").toLowerCase();
  if (s.includes("π") || s.includes("pi")) {
    const part = s.replace("pi", "π");
    const sign = part.startsWith("-") ? -1 : 1;
    const core = part.replace("-", "");
    const [numPart, denPart] = core.split("/");
    const beforePi = numPart.split("π")[0];
    const num = beforePi === "" ? 1 : (beforePi === "-" ? -1 : Number(beforePi));
    const den = denPart ? Number(denPart) : 1;
    return sign * (num * PI) / den;
  }
  const x = Number(s);
  if (!isNaN(x)) return x; // déjà en radians
  return null;
}
...
// angles remarquables (0..2π) — multiples de π/6, π/4, π/3, π/2
const baseAngles = [
...
// Questions mental / identités simples
const IDENTITY_QUESTIONS = [
  { q: "sin(π + x) = ?", options: ["-sin x","sin x","-cos x"], answer: 0, tip:"Symétrie centrale (π) ⇒ signe de sin change." },
  { q: "sin(π - x) = ?", options: ["-sin x","sin x","cos x"], answer: 1, tip:"Symétrie axe Oy: sin conserve le signe en Q2." },
  { q: "cos(π - x) = ?", options: ["-cos x","cos x","-sin x"], answer: 0, tip:"En Q2, cos devient négatif." },
  { q: "cos(π + x) = ?", options: ["-cos x","cos x","-sin x"], answer: 0, tip:"Translation de π ⇒ cos change de signe." },
  { q: "sin(−x) = ?", options:["-sin x","sin x","cos x"], answer:0, tip:"sin impaire." },
  { q: "cos(−x) = ?", options:["-cos x","cos x","sin x"], answer:1, tip:"cos paire." },
  { q: "sin(π/2 − x) = ?", options:["cos x","sin x","-cos x"], answer:0, tip:"Complémentaire." },
  { q: "cos(π/2 − x) = ?", options:["sin x","cos x","-sin x"], answer:0, tip:"Complémentaire." },
];
...
function Card({children}){
  return <div className="rounded-2xl shadow-md p-4 bg-white">{children}</div>
}
...
function TrigoCircle({angle, setAngle, showGuides=true}){
  ...
  function setFromEvent(e){
    ...
    const dx = x - cx, dy = cy - y; // y inversé
    ...
    <div ref={ref} className="w-full flex justify-center">
      ...
      >
        ...
        {baseAngles.map((a,i)=>{
          ...
          return (
            <g key={i}>
              <line x1={ix} y1={iy} x2={tx} y2={ty} stroke="#d1d5db" />
            </g>
          );
        })}
        {showGuides && (
...
function Flashcards({progress, updateProgress}){
  ...
  return (
    <Card>
      ...
      <div className={`rounded-xl border ${flipped?"bg-gray-900 text-white":"bg-white"} p-6 text-center active:scale-[.99] transition`} onClick={()=>setFlipped(!flipped)}>
...
function VisualMode({progress, updateProgress}){
  ...
  function exactValue(kind, a){
    const t = 1/Math.sqrt(2); // √2/2
    const u = Math.sqrt(3)/2; // √3/2
    const r = normalizeAngle(a);
    const ra = [0, PI/6, PI/4, PI/3, PI/2];
    const vals = {
      sin: {
        0: 0, [PI/6]: 1/2, [PI/4]: t, [PI/3]: u, [PI/2]: 1
      },
      cos: {
        0: 1, [PI/6]: u, [PI/4]: t, [PI/3]: 1/2, [PI/2]: 0
      }
    };
    function valLabel(x){
    ...
    let ref = r;
    if (quad===2) ref = PI - r;
    else if (quad===3) ref = r - PI;
    else if (quad===4) ref = TAU - r;
    let closest = ra.reduce((p,c)=> Math.abs(c-ref) < Math.abs(p-ref)? c: p, 0);
    let base = vals[kind][closest];
    if (kind==="sin"){
      if (quad===3||quad===4) base = -base;
    } else {
      if (quad===2||quad===3) base = -base;
    }
  ...
  function distractors(trueLabel){
    ...
    while (out.length<3){
      ...
      if (!out.includes(c) && c!==trueLabel) out.push(c);
    }
    return out;
  }
  ...
  function answer(idx){
    ...
    newProg.history.push({t:Date.now(), score:newProg.correct/newProg.attempts});
    updateProgress(newProg);
    setTarget(makeVisualQuestion());
    setAngle(randChoice(baseAngles));
  }

  return (
    <Card>
      <SectionTitle>Exercices visuels — cercle trigonométrique</SectionTitle>
      <div className="text-sm text-gray-500 mb-2">Déplace le point sur le cercle pour ressentir l'angle. Réponds au quiz ci-dessous.</div>
      ...
      <div className="mt-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {target.options.map((opt,idx)=>
            <button key={idx} onClick={()=>answer(idx)} className="px-3 py-2 rounded-xl border border-gray-300 active:scale-[.99]">{opt}</button>
          )}
        </div>
      </div>
    </Card>
...
function StepsMode({progress, updateProgress}){
  ...
  const steps = useMemo(()=>{
    ...
    const sign = f.includes("π+x")? -1 : f.includes("π−x")||f.includes("π-x")? (isSin? +1 : -1) : (isSin? +1 : +1);
    const out = [];
    ...
    if (f.includes("π−x")||f.includes("π-x")){
      out[1] = { desc: "Symétrie par rapport à Oy (π − x).", angle: normalizeAngle(PI - x) };
    }
    const sVal = Math.sin(x), cVal = Math.cos(x);
    const result = isSin ? sign * sVal : sign * cVal;
    out.push({ desc: `Conclusion: ${f} = ${sign<0?"-":""}${isSin?"sin x":"cos x"}.`, angle: out.at(-1).angle, result });
    return out;
  },[x, formula]);
  ...
  return (
    <Card>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {["sin(π − x)", "sin(π + x)", "cos(π − x)", "cos(π + x)", "sin(π/2 − x)", "cos(π/2 − x)"].map(f=>
            <button key={f} className={`px-3 py-2 rounded-xl border ${formula===f?"bg-black text-white border-black":"border-gray-300"}`} onClick={()=>{setFormula(f); setStep(0);}}>{f}</button>
          )}
        </div>
        <label className="text-sm">Choisis x</label>
        <input type="range" min={0} max={TAU} step={0.01} value={x} onChange={e=>{setX(Number(e.target.value)); setStep(0);}} />
        <div className="text-sm text-gray-500">x = {radToNiceLabel(x)}</div>
        <div className="rounded-xl border border-gray-200 p-3">
          <div className="font-medium mb-2">Étape {step+1} / {steps.length}</div>
          <TrigoCircle angle={steps[step].angle} setAngle={()=>{}} />
          <div className="mt-2">{steps[step].desc}</div>
          {steps[step].result!==undefined && (
            <div className="mt-2 text-sm text-green-600">Résultat numérique (pour x choisi) : {(steps[step].result).toFixed(3)}</div>
          )}
          <div className="mt-3 flex gap-2">
            <button className="px-3 py-2 rounded-xl border border-gray-300" onClick={()=>setStep(Math.max(0, step-1))}>Précédent</button>
            <button className="px-3 py-2 rounded-xl bg-black text-white" onClick={()=> setStep(Math.min(steps.length-1, step+1))}>Suivant</button>
...
function EquationsMode({progress, updateProgress}){
  ...
  function makeEqQuestion(){
    ...
    function toBase(term){
      ...
      if (term === "sin x") return { kind:"sin", sign:1 };
      if (term === "cos x") return { kind:"cos", sign:1 };
      if (term.includes("π − x")||term.includes("π - x")){
        return { kind: isSin?"sin":"cos", sign: isSin? +1 : -1 };
      }
      if (term.includes("π + x")){
        return { kind: isSin?"sin":"cos", sign: -1 };
      }
      if (term.includes("π/2 − x")||term.includes("π/2 - x")){
        return { kind: isSin?"cos":"sin", sign: +1 };
      }
      return { kind:isSin?"sin":"cos", sign:1 };
    }
    ...
    function solutions(){
      const sols = [];
      const grid = [];
      for (let a of baseAngles) grid.push(a);
      for (let a of grid){
        const s = Math.sin(a), c = Math.cos(a);
        const leftVal = (L.kind==="sin"? s: c) * L.sign;
        const rightVal = (R.kind==="sin"? s: c) * R.sign;
        if (Math.abs(leftVal - rightVal) < 1e-6) sols.push(a);
      }
      const uniq=[]; sols.forEach(a=>{ if(!uniq.some(b=>approxEqual(a,b))) uniq.push(a)});
    ...
    function fmtList(list){
      ...
      return list.map(radToNiceLabel).join(", ");
    }

    const correctList = fmtList(sols);
    const fake1 = fmtList(randSubset(baseAngles, Math.max(0, sols.length-1)));
    const fake2 = fmtList(randSubset(baseAngles, Math.min(3, sols.length+1)));
    const fake3 = fmtList(randSubset(baseAngles, Math.min(2, sols.length)));
    const options = shuffle([correctList, fake1, fake2, fake3]);

    return { eqLabel, sols, options, answerIdx: options.indexOf(correctList) };

    function shuffle(a){ const b=[...a]; for(let i=b.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1)); [b[i],b[j]]=[b[j],b[i]];} return b; }
    function randSubset(arr, k){ const b=[...arr]; const out=[]; while(b.length && out.length<k){ const i=Math.floor(Math.random()*b.length); out.push(b.splice(i,1)[0]); } return out; }
  }
  ...
  return (
    <Card>
      ...
      <div className="mt-2 text-xs text-gray-500">Exemples : cos x = sin(π + x) ⇒ cos x = -sin x ⇒ tan x = -1 ⇒ x = 3π/4, 7π/4.</div>
    </Card>
  );
}
...
function ProgressView({store}){
  const data = useMemo(()=>{
    const arr=[];
    for (const [mode, obj] of Object.entries(store.progress)){
  ...
  const summary = Object.fromEntries(Object.entries(store.progress).map(([k,v])=>[
    k, { rate: v.attempts? Math.round(100*v.correct/v.attempts): 0, attempts: v.attempts }
  ]));
      ...
      <div className="mt-4 h-56">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{left:12,right:12}}>
            ...
            <XAxis dataKey="label" interval={Math.max(0, Math.floor(data.length/6))} />
...
export default function TrigoTrainer(){
  ...
  function updateProgress(mode){
    return (newProg)=> setStore(s=> ({ ...s, progress: { ...s.progress, [mode]: newProg } }));
  }
    ...
    <div className="min-h-screen text-gray-900 p-4 max-w-xl mx-auto">
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {[
          ...
          ["visual","Cercle (visuel)"],
          ["steps","Étapes guidées"],
          ["equations","Équations"],
      ...
      {tab==="flashcards" && (
        <Flashcards progress={store.progress.flashcards} updateProgress={updateProgress("flashcards")} />
      )}
      {tab==="visual" && (
        <VisualMode progress={store.progress.visual} updateProgress={updateProgress("visual")} />
      )}
      {tab==="steps" && (
        <StepsMode progress={store.progress.steps} updateProgress={updateProgress("steps")} />
      )}
      {tab==="equations" && (
        <EquationsMode progress={store.progress.equations} updateProgress={updateProgress("equations")} />
      )}
      {tab==="progress" && (
